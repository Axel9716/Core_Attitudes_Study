...
In this section, I select a range of questions and add category names to them based on the questionnaire guide on the WVS website. I confirmed there were no null values, because for every variable there either shouldn't be null values or they are encoded as negative numbers. I am also removing `q172r` because it is less granular version of `q172` that is recoded post-survey. Lastly, I recoded the negative values for the questions as NA, so the data will be easier to work with later on.
